...
object SelectorMacroCaller {
  ...
  def foreachImpl(c:Context)(x: c.Expr[SelectorContext=>Unit]):c.Expr[Unit] =
  {
   ...
   val tree = Block(List(ValDef(Modifiers(), newTermName("f"), TypeTree(), xtree)), Apply(Ident(newTermName("f")), List(Apply(Select(New(Select(Select(Ident(newTermName("go")), newTermName("channels")), newTypeName("SelectorContext"))), nme.CONSTRUCTOR), List()))))
